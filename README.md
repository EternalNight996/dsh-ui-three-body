...
<p align="center">
  <img src="assets/screen/sophon-preview.png" width="640" alt="智子菜单与皮肤切换" />
  <br/>
  <em>智子菜单：五步纲领 / 需求剖析 / 皮肤循环切换 / 休眠</em>
</p>
...
| 亮点 | 说明 |
| --- | --- |
| 🧠 **AI 真正开智** | 每次对话注入「驯兽内核」——第一性原理 + **五步纲领**（①问清→②方案→③章程→④执行→⑤交付），一次问清、一次定稿、绝不反复问 |
| 🎨 **方案呈现规范** | 章程先声明交付形态：GUI/Web **优先附 HTML 原型** + 设计 token + 组件清单；先定风格方向 + 审美锚点，让审核看得见、开发不抓瞎 |
...
| 📊 **头顶进度条** | 智子头顶常驻进度条，点击展开/收起——显示「驯兽五步 + 当前状态（驭兽中/待命）」，进度一眼可见 |
...
| --- | --- | --- |
| `minimal` | ~113 | 一键五步骨 |
| `balanced`（默认） | ~290 | 三铁律 + **五步纲领**（问清→方案→章程→执行→交付） |
| `full` | ~384 | balanced + 官网示例 |
