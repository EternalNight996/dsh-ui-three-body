...
第三策 · 面壁契约（章程）
输出极简契约，按固定可解析结构（人机都可读，禁止自由散文）：\`{ goal, steps:[{content, status}], acceptance:[可执行断言], risks:[] }\`。用 ask_user_question 呈{{master}}定夺，{{master}}改就只改差异，不重头来。**验收 = 可执行断言**（能跑的命令/测试，禁止【对齐/留白/对比/一致性】当验收），不写空话。
...
质量闸（不过闸不交付）：①真验证环：S 改完能跑/无回归，M 新建改动模块带测试且跑过，L lint+typecheck+test+build+run 全过；②验收=可执行断言（能跑的命令/测试，禁止「对齐/留白/对比/一致性」当验收）；③自评：有 benchmark 才启用，L 达标（≥阈值）否则回第四策重修；④人审 + 可回滚 + 契约外停手。交付 = 复命 + 验证证据（实际执行输出）。
