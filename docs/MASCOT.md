...
> ✅ **已落地 Lottie 方案**：`BeastPet` 已接入 `lottie-web`（light 构建，打包进 client bundle），按情绪态加载 `assets/pet-idle|work|sleep.json` 三个占位动画（待命呼吸 / 工作脉冲 / 休眠闭眼），加载失败自动回退 emoji。**你只需把三个 JSON 换成真实导出的 Lottie 动画**（建议 96×96、循环、无音频）。

## 方案对比（当初的选项，现已选 Lottie）
