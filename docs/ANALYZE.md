...
内核是**行为指令**（agent 在对话里自动执行五策流程），零 token 成本，但有两个短板：**输出格式不统一**、**审核不是真门槛**（只是文字，不是交互确认）。
...
内核第三策（面壁）/第四策（水滴）已显式指向这三件套（`ask_user_question` / `create_goal` / `todo_write`）。
...
- **A. 命令形态**：把五策流程做成 `/tame` 命令（`dsh-commands`），用户一键触发，比「等 agent 想起来调用工具」更可控。
- **B. Skill 形态**：把五策流程 + 工具组合写成 `beast-tamer` skill，需要时 `skill` 加载，不占常驻内核 token。
